...
# pprint(primes, indent=4);

mul = lambda x, y: x*y

# maybe i can reuse the primefactorisation code !

# https://youtu.be/YFNdv60wD5I?t=473
...
def findLCM(nList, primesIndex = 0, results = []):
    ...
    for i in nList:
        ...
        if i % divisor == 0:
            # if any number can be divided by a number, we divide all the go to next level,
            # note: but do e really need to use only prime numbers ?

            # divide all !
            # nList = [n//divisor for n in nList if n % divisor == 0]
            nList = [n//divisor if n % divisor == 0 else n for n in nList]
            results.append(divisor)

            return findLCM(nList, primesIndex, results)
        # if no number can be divided than we have our LCM
